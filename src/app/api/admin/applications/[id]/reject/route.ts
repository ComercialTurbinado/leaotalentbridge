...
export async function POST(
  ...
  try {
    ...
    try {
      await sendNotification(updatedApplication, 'rejected', reason);
...
async function sendNotification(application: any, decision: string, reason: string) {
  try {
    ...
    const candidateNotification = {
      ...
      message: `Sua candidatura para ${application.jobId.title} foi rejeitada pelo administrador.`,
      data: {
    ...
    const companyNotification = {
      ...
      type: 'application_decision',
      title: 'Candidatura Rejeitada',
      message: `A candidatura de ${application.candidateId.name} para ${application.jobId.title} foi rejeitada pelo administrador.`,
      data: {
        applicationId: application._id,
