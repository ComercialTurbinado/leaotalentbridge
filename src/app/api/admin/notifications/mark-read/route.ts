...
// PUT - Marcar múltiplas notificações como lidas
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `${result.modifiedCount} notificação(ões) marcada(s) como lida(s) com sucesso`,
    ...
    return NextResponse.json(
      { error: 'Erro interno do servidor ao marcar notificações como lidas' },
      { status: 500 }
    );
  }
}
